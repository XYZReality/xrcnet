...
parser.add_argument('--checkpoint', type=str, default='../trained_models/xrcnet.pth')
parser.add_argument('--experiment_name', type=str, default='xrcnet')
...
parser.add_argument('--output_dir', type=str, default='.', help='where to store the result of the evaluation')
...
if not args.benchmark:
    if not os.path.exists(os.path.realpath(args.output_dir)):
        os.makedirs(os.path.realpath(args.output_dir))
    ...
    out = open(os.path.join(os.path.realpath(args.output_dir),output_name), 'w')
    print(output_name)
...
with torch.no_grad():
    for i in range(0*6, len(im_list), 6):
        if args.benchmark:
            i = 0
        ...
        for j in range(1, len(scene_list)):     # pair the first image with the rest of 5 images
            ...
            if not args.benchmark:
                # save matches
                save_dir = os.path.join(os.path.realpath(args.output_dir), experiment_name)
                try:
                    os.mkdir(save_dir)
