...
parser.add_argument('--method_name', type=str, default='xrcnet', help='Name of the method')
parser.add_argument('--output_dir', type=str, default='.', help='Path to store the output images at')
args = parser.parse_args()

output_dir = os.path.join(args.output_dir,"Aachen_{}".format(args.method_name))
...
for pair in image_pairs[0:]:
    ...
    pdf_doc = '%s_vs_%s.pdf' % (name1, name2)
    png_img = '%s_vs_%s.png' % (name1, name2)
    plt.savefig(os.path.join(output_dir, pdf_doc),bbox_inches='tight')
    plt.savefig(os.path.join(output_dir, png_img), bbox_inches='tight')
    plt.close()
