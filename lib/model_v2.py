...
class FeatureExtraction(torch.nn.Module):
    def __init__(self, train_fe=False, feature_extraction_cnn='resnet101', feature_extraction_model_file='',
        ...
        if feature_extraction_cnn == 'resnet18fpn_3_256_4':    # feature with 256 channels
            # left only one 3x3 conv after the final merge
    ...
    def forward(self, image_batch):
        ...
        elif self.feature_extraction_cnn == 'resnet18fpn_3_256_4':
            bottom_up = []
            for extractor in self.extractor_list:
                image_batch = extractor(image_batch)
                bottom_up.append(image_batch)
...
class NeighConsensus_no_learnables(torch.nn.Module):
    def __init__(self):
        super(NeighConsensus_no_learnables, self).__init__()
...
class ImMatchNet(nn.Module):
    def __init__(self,
        ...
        if checkpoint is not None and checkpoint is not '':
            ...
            if hasattr(checkpoint['args'], 'config'):
            
                config_path = os.path.join(
                    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 
                    'configs/{}'.format(os.path.basename(checkpoint['args'].config))
                )
                ckpt_config = lib.tools.parseConfig(config_path)


                print('Using checkpoint parameters: ')
                if hasattr(ckpt_config, 'backbone'):
                    feature_extraction_cnn = ckpt_config.backbone
                ncons_channels = ckpt_config.NCNet["channels"]
                print('  ncons_channels: ' + str(ncons_channels))
                ncons_kernel_sizes = ckpt_config.NCNet["kernel_sizes"]
                print('  ncons_kernel_sizes: ' + str(ncons_kernel_sizes))

                #  get_backbone_after_stage=cfg., with_transition=cfg.
                if hasattr(ckpt_config, 'get_backbone_after_stage'):
                    get_backbone_after_stage = ckpt_config.get_backbone_after_stage

                if hasattr(ckpt_config, 'with_transition'):
                    with_transition = ckpt_config.with_transition

                if hasattr(ckpt_config, 'NCNet'):
                    if 'learned' in ckpt_config.NCNet:
                        nc_learnable = ckpt_config.NCNet["learned"]
        ...
        if nc_learnable:
            print('Using NC Module w/ Learnable Parameters')
            self.NeighConsensus = NeighConsensus(use_cuda=self.use_cuda,
                                                kernel_sizes=ncons_kernel_sizes,
                                                channels=ncons_channels)
        else:
            print('Using NC Module w/o Learnable Parameters')
            self.NeighConsensus = NeighConsensus_no_learnables()
        ...
        if self.half_precision:
            ...
            if hasattr(self.NeighConsensus, 'conv'):
