...
class Config:
    def __init__(self, params_dict):
        self.image_size = params_dict.get("image_size", 400)
        self.im_fe_ratio = params_dict.get("im_fe_ratio", 16)
        self.fine_coarse_ratio = params_dict.get("fine_coarse_ratio", 4)
        self.gauss_size = params_dict.get("gauss_size", 3)
        self.numKey = params_dict.get("numKey", 400)

        self.NCNet = params_dict.get("NCNet", dict())
        self.NCNet["kernel_sizes"] = self.NCNet.get("kernel_sizes", [3,3])
        self.NCNet["channels"] = self.NCNet.get("channels", [16, 1])
        self.NCNet["learned"] = self.NCNet.get("learned", True)
        
        self.training = params_dict.get("training", dict())
        self.training["num_epochs"] = self.training.get("num_epochs", 15)
        self.training["num_workers"] = self.training.get("num_workers", 4)
        self.training["batch_size"] = self.training.get("batch_size", 16)
        self.training["use_writer"] = self.training.get("use_writer", True)
        self.training["multi_gpu"] = self.training.get("multi_gpu", True)
        self.training["fe_finetune_params"] = self.training.get("fe_finetune_params", 0)
        
        self.backbone = params_dict.get("backbone", "resnet101fpn_3_1024_4")

        self.loss = params_dict.get("loss", dict())
        self.loss["weight_loss"] = self.loss.get("weight_loss", [0, 1, 0])
        self.loss["mode"] = self.loss.get("mode", 1)
        self.loss["loss"] = self.loss.get("loss", "meanfnorm")
        self.loss["weight_orthogonal"] = self.loss.get("weight_orthogonal", 0.05)

        self.optimizer = params_dict.get("optimizer", dict())
        self.optimizer["type"] = self.optimizer.get("type", "adam")
        self.optimizer["momentum"] = self.optimizer.get("momentum", 0.9)
        self.optimizer["lr"] = self.optimizer.get("lr", 0.01)
        self.optimizer["use_scheduler"] = self.optimizer.get("use_scheduler", True)
        self.optimizer["scheduler_milestone"] = self.optimizer.get("scheduler_milestone", [5, 10, 15])

def parseConfig(json_path):
...
class ImgMatcher:
    ...
    def __init__(self, checkpoint, use_cuda, half_precision, im_fe_ratio = 16, postprocess_device = None):
        ...
        if hasattr(checkpoint_['args'], 'config'):
            config_path = os.path.join(
                os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 
                'configs/{}'.format(os.path.basename(checkpoint_['args'].config))
            )
            ckpt_config = parseConfig(config_path)
            multi_gpu = ckpt_config.training["multi_gpu"]

            if hasattr(ckpt_config, "fine_coarse_ratio"):
                fine_coarse_ratio = ckpt_config.fine_coarse_ratio
        else:
            multi_gpu = checkpoint_['args'].multi_gpu
            fine_coarse_ratio = checkpoint_['args'].fine_coarse_ratio
