...
parser.add_argument('--checkpoint', type=str, default='../trained_models/xrcnet.pth.tar')
...
for q in queries_idx_chunk:
    ...
    # load database image
        for idx in indices:
            ...
            # try:
            #     os.mkdir('/data/storage/inloc_%s_%i/' % (args.experiment_name, 500))
            # except FileExistsError:
            #     pass
            # fn = '%s_vs_%s.png' % (db[q][0].item(), db[q][1].ravel()[idx].item().replace('/', '-'))
            # fn2 = '%s_vs_%s.pdf' % (db[q][0].item(), db[q][1].ravel()[idx].item().replace('/', '-'))
            # plt.savefig(os.path.join('/data/storage/inloc_%s_%i/' % (args.experiment_name, 500) , fn),bbox_inches='tight')
            # plt.savefig(os.path.join('/data/storage/inloc_%s_%i/' % (args.experiment_name, 500) , fn2),bbox_inches='tight')
            # plt.close()
